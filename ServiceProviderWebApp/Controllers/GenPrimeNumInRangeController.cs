...
{
    /**
     * GenPrimeNumInRangeController is a ASP.NET Web API controller class
     * It has a Get method
     */
    public class GenPrimeNumInRangeController : ApiController
    {
        /**
         * This rest service generates prime numbers in a range (two inputs) and returns the output in JSON
         * 
         ...
         */
        public List<int> Get(int numOne, int numTwo, int token)
        {
            ...
            //if the datamodel is valid, it set result to the prime number between numOne and numTwo
            else
            {
                //loop from numOne to less than or equal to numTwo, and increment primeNum by 1 each time
                for (int primeNum = numOne; primeNum <= numTwo; primeNum++)
                {
                    //set numIndex to 0
                    int numIndex = 0;
